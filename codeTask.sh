    ...
    for i in {1..50}; do
        case $((RANDOM % 5)) in
            0)
                ...
                echo -e "\nexport const ${var_name}_${timestamp}_${i} = 'autogenerated_${i}';" >> "$TS_FILE_PATH"
                ;;
            1)
                # 添加类型定义
                echo -e "\ntype AutoGeneratedType_${timestamp}_${i} = string | number | boolean;" >> "$TS_FILE_PATH"
                ;;
            2)
                # 添加接口定义
                echo -e "\ninterface ExtendedTrackingData_${timestamp}_${i} { readonly id: string; timestamp: number; data: Record<string, unknown>; }" >> "$TS_FILE_PATH"
                ;;
            3)
                # 添加导出函数
                echo -e "\nexport const getTrackingInfo_${timestamp}_${i} = () => ({ timestamp: ${timestamp}, index: ${i} });" >> "$TS_FILE_PATH"
                ;;
            4)
                # 添加注释行
                echo -e "\n// 类型不明确，需要同后端确认类型 ${i} - ${date_str}" >> "$TS_FILE_PATH"
                ;;
        esac
    done
